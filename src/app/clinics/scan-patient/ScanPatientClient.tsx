...
const MRI_META: Record<string, { label: string; color: string; bg: string }> = {
  ...
  unknown:     { label: 'Unknown status', color: 'var(--muted)', bg: 'color-mix(in srgb,var(--muted) 10%,transparent)'  },
}
...
export default function ScanPatientClient() {
  ...
  function extractIidCode(raw: string): string | null {
    return raw.match(/IID-[A-Z0-9]{6,}/i)?.[0].toUpperCase() ?? null
  }
  ...
  const scanFrame = useCallback(() => {
    ...
    if (!ctx || !video.videoWidth) { rafRef.current = requestAnimationFrame(scanFrame); return }
    ...
    import('jsqr').then(({ default: jsQR }) => {
      ...
      if (code?.data) {
        ...
        if (iid) { stopCamera(); setInputCode(iid); setSearchCode(iid); return }
      }
  ...
  async function startCamera() {
    ...
    try {
      ...
      setVideoMirrored(settings?.facingMode === 'user' || !settings?.facingMode)
      setCameraActive(true)
      rafRef.current = requestAnimationFrame(scanFrame)
    } catch {
      setCameraError('Camera unavailable. Enter the code below instead.')
    }
  }
  ...
  function handleSearch() {
    ...
    if (cleaned) { setSearchCode(cleaned); setExpandedDeviceId(null) }
  }

  function handleKeyDown(e: React.KeyboardEvent) { if (e.key === 'Enter') handleSearch() }

  function handleClear() {
    setInputCode(''); setSearchCode(''); setExpandedDeviceId(null)
  ...
  const mriKey    = found && result.verificationStatus === 'active' ? (result.mriStatus ?? 'conditional') : 'unknown'
  const mriMeta   = MRI_META[mriKey] ?? MRI_META.unknown
    ...
    <div className="m-content scan-page">
      ...
      <div className="m-h" style={{ marginBottom: 28 }}>
        <div>
          ...
          <div className="sub">Scan a QR code or enter an Implant ID to retrieve MRI safety information.</div>
        </div>
      </div>

      {/* ── Camera viewfinder ── */}
      <div className={`viewfinder${cameraActive ? ' scanning' : ''}`} style={{ maxWidth: 480 }}>
        <video
          ref={videoRef}
          playsInline
          muted
        ...
        {cameraActive && <div className="vf-scan-line" aria-hidden="true" />}
        ...
        {!cameraActive && (
          <div className="vf-idle">
            ...
            <p>Point camera at the QR code on the patient&rsquo;s Implant ID card</p>
          </div>
        )}
      </div>

      {/* ── Camera controls ── */}
      <div style={{ maxWidth: 480, marginBottom: 4, marginTop: 12 }}>
        {!cameraActive ? (
          <button type="button" className="btn btn-s btn-block" onClick={startCamera}
      ...
      {cameraError && (
        <p style={{ color: 'var(--err)', fontSize: 13, fontFamily: 'var(--ff)', maxWidth: 480, margin: '8px 0 0' }}>
          {cameraError}
        </p>
      )}

      {/* ── Divider ── */}
      <div className="scan-or" style={{ maxWidth: 480, margin: '20px 0' }}>or enter code</div>

      {/* ── Search field ── */}
      <div style={{ maxWidth: 480, marginBottom: 32 }}>
        <label htmlFor="patient-code" style={{ display: 'block', fontFamily: 'var(--ff)', fontSize: 12.5, fontWeight: 500, color: 'var(--muted)', marginBottom: 8, letterSpacing: '.2px' }}>
          Implant ID code
        </label>
        <div style={{ display: 'flex', gap: 10 }}>
          <div style={{ position: 'relative', flex: 1 }}>
            ...
            />
            {inputCode && (
              <button type="button" onClick={handleClear} aria-label="Clear" style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--muted2)', padding: 0, lineHeight: 1 }}>
          ...
          <button type="button" className="btn btn-s" onClick={handleSearch} disabled={!inputCode.trim()}>
            Search
          </button>
        </div>
      </div>
      ...
      {/* ── Loading ── */}
      {isLoading && (
        <div style={{ color: 'var(--muted)', fontSize: 14, fontFamily: 'var(--ff)' }}>Looking up record…</div>
      )}

      {/* ── Not found ── */}
      {notFound && (
        <div style={{ background: 'color-mix(in srgb,var(--err) 6%,transparent)', border: '1px solid color-mix(in srgb,var(--err) 18%,transparent)', borderRadius: 12, padding: '20px 24px', maxWidth: 560, display: 'flex', alignItems: 'flex-start', gap: 14 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 13.5, color: 'var(--muted)' }}>
              No patient found with code <strong style={{ fontFamily: 'SF Mono,Monaco,monospace' }}>{searchCode}</strong>. Check the code and try again.
            </div>
          </div>
        </div>
      )}
      ...
      {/* ── Result card ── */}
      {found && (
        <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 16, padding: '28px', maxWidth: 560 }}>

          {/* Found badge */}
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'color-mix(in srgb,var(--ok) 10%,transparent)', border: '1px solid color-mix(in srgb,var(--ok) 22%,transparent)', borderRadius: 8, padding: '5px 12px', fontFamily: 'var(--ff)', fontSize: 12.5, fontWeight: 600, color: 'var(--ok)', marginBottom: 20 }}>
          ...
          {/* MRI status */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: mriMeta.bg, border: `1px solid color-mix(in srgb,${mriMeta.color} 22%,transparent)`, borderRadius: 10, padding: '10px 16px', marginBottom: 24 }}>
            ...
            <span style={{ fontFamily: 'var(--ff)', fontSize: 14, fontWeight: 700, color: mriMeta.color }}>{mriMeta.label}</span>
          </div>

          {/* Patient fields */}
          <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: '10px 24px', marginBottom: deviceLinks?.length ? 24 : 0 }}>
            <div style={{ color: 'var(--muted)', fontSize: 12.5, fontFamily: 'var(--ff)' }}>Implant ID</div>
            <div style={{ fontFamily: 'SF Mono,Monaco,monospace', fontSize: 13, fontWeight: 600, color: 'var(--accent)' }}>{result.implantIdCode}</div>

            <div style={{ color: 'var(--muted)', fontSize: 12.5, fontFamily: 'var(--ff)' }}>Name</div>
            <div style={{ fontFamily: 'var(--ff)', fontSize: 13.5, fontWeight: 500, color: 'var(--text)' }}>{result.firstName} {result.lastName}</div>
            ...
            <div>
              <span style={{ fontFamily: 'var(--ff)', fontSize: 11.5, fontWeight: 600, padding: '2px 8px', borderRadius: 5, background: result.verificationStatus === 'active' ? 'color-mix(in srgb,var(--ok) 10%,transparent)' : 'color-mix(in srgb,#f59e0b 10%,transparent)', color: result.verificationStatus === 'active' ? 'var(--ok)' : '#92400e', border: result.verificationStatus === 'active' ? '1px solid color-mix(in srgb,var(--ok) 25%,transparent)' : '1px solid color-mix(in srgb,#f59e0b 25%,transparent)' }}>
                ...
                return (
                  <div key={d._id} style={{ border: '1px solid var(--border)', borderRadius: 10, overflow: 'hidden', marginBottom: 8 }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', background: 'var(--bg)' }}>
                      ...
                      <button type="button" onClick={() => setExpandedDeviceId(isExpanded ? null : d.deviceId)} aria-expanded={isExpanded}
                        style={{ background: 'transparent', border: '1px solid var(--border)', borderRadius: 6, cursor: 'pointer', color: 'var(--muted)', fontFamily: 'var(--ff)', fontSize: 11.5, fontWeight: 500, padding: '4px 10px', transition: 'all .12s' }}>
                    ...
                    {isExpanded && (
                      <div style={{ borderTop: '1px solid var(--border)', padding: '16px', background: 'var(--bg2)' }}>
                        {deviceDetail === undefined ? (
                          <div style={{ color: 'var(--muted)', fontSize: 13, fontFamily: 'var(--ff)' }}>Loading parameters…</div>
                        ) : deviceDetail === null ? (
                          <div style={{ color: 'var(--muted)', fontSize: 13, fontFamily: 'var(--ff)' }}>Parameters not available.</div>
                        ) : (
                          <>
                            {deviceDetail.mriStatus === 'conditional' && (
                              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, background: 'color-mix(in srgb,#f59e0b 8%,transparent)', border: '1px solid color-mix(in srgb,#f59e0b 22%,transparent)', borderRadius: 8, padding: '10px 14px', marginBottom: 14 }}>
