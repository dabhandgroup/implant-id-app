  ...
  return (
    <div className={`custom-select${open ? ' open' : ''}`} ref={ref} style={style}>
      <button ref={btnRef} type="button" tabIndex={0} className="custom-select-btn"
        onKeyDown={handleKeyDown} onClick={() => { setOpen(o => !o); btnRef.current?.focus() }}>
...
export default function NewPatientClient() {
  const addPatient = useMutation(api.patients.clinicAddPatient)
  ...
  const [success,   setSuccess]   = useState<{ implantIdCode: string } | null>(null)
  ...
  function goToImplant(e: React.FormEvent) {
    ...
    if (!email.trim() || !email.includes('@'))
      return showErr('Please enter the patient\'s email address')
    goStep('implant')
  }
  ...
  async function doSubmit() {
    ...
    try {
      ...
      setSuccess({ implantIdCode: result.implantIdCode })
    } catch (ex) {
  ...
  if (success) {
    return (
      <div className="m-content">
        <div className="ap-success-page" style={{ padding: '32px 0 80px' }}>
          <div className="ap-success-card">
            ...
            <p className="ap-success-sub">
              <strong>{firstName} {lastName}</strong> has been added to the platform.
            </p>
            ...
            <div className="ap-success-notice ok">
              ...
              <span>A welcome email is being sent to <strong>{email}</strong> with their Implant ID and a sign-in link.</span>
            </div>

            <div className="ap-success-actions">
              <button className="btn btn-lg" onClick={resetForm}>+ Add another patient</button>
              <button className="btn btn-s btn-lg" onClick={() => router.push('/master/patients')}>
                View all patients →
              </button>
            </div>
          </div>
  ...
  return (
    <div className="m-content">
      ...
      <div className="ap-page" style={{ padding: 0, maxWidth: 740 }}>
