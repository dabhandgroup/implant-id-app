...
function CopyBadge({ text }: { text: string }) {
  ...
  return (
    <button
      ...
      style={{ display:'inline-flex', alignItems:'center', gap:6, fontFamily:'SF Mono,Monaco,monospace', fontSize:13, fontWeight:600, letterSpacing:'.06em', color:'var(--accent-deep)', background:'color-mix(in srgb,var(--accent) 10%,transparent)', border:'1px solid color-mix(in srgb,var(--accent) 22%,transparent)', borderRadius:8, padding:'4px 10px', cursor:'pointer' }}
    >
      {text}
      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" aria-hidden="true">
        {copied ? <path d="m9 12 2 2 4-4"/><circle cx="12" cy="12" r="9"/> : <><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></>}
      </svg>
    </button>
  )
}
...
export default function DeviceDetailClient({ id }: { id: string }) {
  ...
  async function handleDelete() {
    ...
    try {
      await deleteDevice({ id: id as Id<'devices'> })
      ...
      <div className="m-h" style={{ marginBottom: 24 }}>
        <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 6, flexWrap: 'wrap' }}>
            <div className="sub" style={{ margin: 0 }}>{device.deviceType} · {device.classification}</div>
            {/* Device code badge */}
            {(device as any).deviceCode && <CopyBadge text={(device as any).deviceCode} />}
          </div>
        </div>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <a href={`/master/devices/${id}/edit`} className="btn">Edit device</a>
