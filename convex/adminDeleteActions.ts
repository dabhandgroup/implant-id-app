...
export const adminRequestClinicDeleteCode = action({
  ...
  handler: async (ctx, args) => {
    ...
    const result = await ctx.runMutation(
      internal.clinics.adminGenerateClinicDeleteCode,
      { applicationId: args.applicationId },
    ) as { code: string; clinicName: string }

    const { code, clinicName } = result
    ...
    const { error } = await resend.emails.send({
      from:    FROM,
      to:      [ADMIN_EMAIL],
      subject: `Clinic deletion code — ${clinicName}`,
      html: `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8"/>
  <meta name="viewport" content="width=device-width,initial-scale=1.0"/>
  <title>Clinic deletion code</title>
  <style>
    body{margin:0;padding:40px 16px;background:#f0f4f8;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif}
    .wrap{max-width:480px;margin:0 auto}
    .head{background:#fff;padding:20px 32px 16px;border-radius:16px 16px 0 0;border-bottom:1px solid #e2e8f0;text-align:center}
...
<body>
  <div class="wrap">
    ...
    <div class="body">
      <h2>Clinic deletion code</h2>
      <p>You requested permanent deletion of <strong>${clinicName}</strong>. Enter the code below to confirm. This will permanently remove the clinic record and all associated staff data.</p>
      <div class="code-box">
        <div class="code-label">Verification code</div>
        <div class="code-value">${code}</div>
        <div class="code-expiry">Expires in 10 minutes</div>
      </div>
      <div class="warn">&#9888; This action is irreversible. All clinic data and staff records will be permanently deleted.</div>
    </div>
    <div class="footer">If you didn&apos;t request this, ignore this email. The code expires automatically.</div>
  </div>
</body>
